...
buildPythonPackage rec {
  ...
  src = fetchGit {
    ...
    name = "chatterbox";
    rev = "249939193ea4cf8a59fcd737bbd2a910a8ebf5ef";
  };

  propagatedBuildInputs = [
    numpy librosa s3tokenizer torch torchaudio
    transformers diffusers conformer safetensors
  ];
  ...
  prePatch = ''
    substituteInPlace pyproject.toml \
      --replace 'torch==2.6.0'        'torch' \
      --replace 'torchaudio==2.6.0'   'torchaudio' \
      --replace 'transformers==4.46.3' 'transformers' \
      --replace 'diffusers==0.29.0'   'diffusers' \
      --replace 'safetensors==0.5.3'   'safetensors'
  '';
}
